...
## 3. Redis를 이용환 동시성 제어
분산락을 구현할 떄 사용하는 대표적인 Redis 라이브러리는 `Lettuce` 와 `Redisson` 이 있다. 

#### Lettuce
- setnx 명령어를 활용하여 분산락 구현
- spin lock 방식
  - 락을 획득하려는 쓰레드가 락을 사용할 수 있는지 반복적으로 확인하면서 락 획득을 시도하는 방식
  - 동시에 많은 스레드가 lock 획득 대기 상태라면 redis에 부하가 갈 수 있다.
  - 재시도 로직 구현해야 함

#### Redisson 
- pub-sub 기반으로 Lock 구현 제공
  - 채널을 하나 만들어 락을 점유 중인 쓰레드가 락을 획득하려고 대기중인 스레드에게 해제를 알려주면 안내를 받은 스레드가 락 획득 시도를 하는 방식이다. 
  - 락 획득 재시도 로직을 기본적으로 제공한다.
  - pub-sub 기반임으로 Lettuce 와 비교했을 때 redis에 부하가 덜 간다.
  - 별도의 라이브러리를 사용한다.

## 실무에서는?
- 재시도가 필요하지 않은 경우에는 Lettuce
- 재시도가 필요한 경우엔 Redisson